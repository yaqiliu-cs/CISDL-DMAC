...
class Test_COCO(object):
    ...
    def test(self):
        ...
        for piece in self.pair_list:            
            ...
            hist1 = fast_hist(gt1.flatten(),output0.flatten(),self.nolabel).astype(float)
            hist2 = fast_hist(gt2.flatten(),output1.flatten(),self.nolabel).astype(float)
...
args.list_path = os.path.join(args.data_path,'val2014_combine_fore_easy.csv')
args.test_num = 3000
...
args.vis_task_path = 'coco_combine_6_easy'

args.score_save_path = 'scores'
args.score_save_filename = 'val2014_combine_fore_easy_dmac_adv.csv'
